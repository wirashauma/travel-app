...
class _CheckoutPageState extends State<CheckoutPage> {
  ...
  Future<void> _proceedToPayment() async {
    ...
    if (_pickupAddress == null || _pickupAddress!.isEmpty) {
      ...
      _showSnack('Silakan pilih alamat penjemputan terlebih dahulu.', isError: true);
      return;
    }
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _sectionTitle('Alamat Penjemputan', Iconsax.location, 320),
                  const SizedBox(height: 10),
                  _buildPickupCard(340),
  ...
  Widget _buildPickupCard(int delayMs) {
    final hasAddress = _pickupAddress != null && _pickupAddress!.isNotEmpty;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: hasAddress ? _C.teal.withValues(alpha: 0.3) : _C.borderLight,
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
          ...
          Row(
            children: [
              Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(
                  color: hasAddress ? _C.teal.withValues(alpha: 0.08) : const Color(0xFFF1F5F9),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Icon(
                  Iconsax.location5,
                  size: 20,
                  color: hasAddress ? _C.teal : _C.textTertiary,
                ),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      hasAddress ? 'Titik Penjemputan Terpilih' : 'Belum Ada Titik Jemput',
                      style: GoogleFonts.plusJakartaSans(
                        fontSize: 14,
                        fontWeight: FontWeight.w700,
                        color: _C.textPrimary,
                    ...
                    const SizedBox(height: 2),
                    Text(
                      hasAddress ? _pickupAddress! : 'Sopir travel akan menjemput langsung ke alamat rumah Anda.',
                      style: GoogleFonts.inter(
                        fontSize: 12,
                        color: hasAddress ? _C.textSecondary : _C.textTertiary,
                      ),
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis,
                    ),
            ...
            child: OutlinedButton.icon(
              onPressed: () async {
                final result = await Navigator.push<Map<String, dynamic>>(
                  context,
                  MaterialPageRoute(
                    builder: (_) => PickLocationPage(originCity: widget.origin),
                  ),
                );
                if (result != null) {
                  setState(() {
              ...
              icon: Icon(
                hasAddress ? Iconsax.edit : Iconsax.map,
                size: 16,
                color: hasAddress ? _C.textSecondary : _C.primary,
              ),
              label: Text(
                hasAddress ? 'UBAH ALAMAT JEMPUT' : 'PILIH ALAMAT RUMAH / JEMPUT',
                style: GoogleFonts.plusJakartaSans(
                  fontSize: 12,
                  fontWeight: FontWeight.w800,
                  color: hasAddress ? _C.textSecondary : _C.primary,
              ...
              style: OutlinedButton.styleFrom(
                side: BorderSide(
                  color: hasAddress ? _C.borderLight : _C.primary.withValues(alpha: 0.3),
                ),
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(10),
                ),
    ...
    ).animate().fadeIn(delay: delayMs.ms, duration: 400.ms).slideY(
          begin: 0.05,
          delay: delayMs.ms,
          duration: 400.ms,
